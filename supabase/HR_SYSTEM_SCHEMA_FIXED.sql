-- HR System Database Schema - Fixed Version
...
-- HR Managers can manage everything (but not create themselves)
CREATE POLICY "HR Managers can view all HR data" ON hr_managers
  FOR SELECT USING (
    EXISTS (
...
INSERT INTO hr_lead_statuses (name, description, color) VALUES
  ('New', 'New lead that needs initial contact', '#3b82f6'),
  ('Contacted', 'Lead has been contacted', '#10b981'),
  ('Hot Lead', 'High priority lead', '#f59e0b'),
  ('Cold Lead', 'Low priority lead', '#6b7280'),
  ('Callback', 'Scheduled for callback', '#8b5cf6'),
  ('Joined', 'Successfully joined', '#059669'),
  ('Not Interested', 'Lead is not interested', '#dc2626'),
  ('Call Not Picked', 'Call was not answered', '#ef4444')
...
CREATE INDEX IF NOT EXISTS idx_hr_leads_assigned_staff ON hr_leads(assigned_staff_id);
CREATE INDEX IF NOT EXISTS idx_hr_leads_status ON hr_leads(status);
CREATE INDEX IF NOT EXISTS idx_hr_lead_activities_lead_id ON hr_lead_activities(lead_id);
