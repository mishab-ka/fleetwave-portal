...
ALTER TABLE categories ADD COLUMN IF NOT EXISTS category_level INTEGER DEFAULT 0;
...
DROP TRIGGER IF EXISTS update_category_path_trigger ON categories;
...
-- Add RLS policies for subcategories (if they don't exist)
DO $$
BEGIN
  -- Users can view all categories
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'categories' AND policyname = 'Users can view all categories') THEN
    CREATE POLICY "Users can view all categories" ON categories
      FOR SELECT USING (true);
  END IF;

  -- Users can insert categories (both main and subcategories)
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'categories' AND policyname = 'Users can insert categories') THEN
    CREATE POLICY "Users can insert categories" ON categories
      FOR INSERT WITH CHECK (true);
  END IF;

  -- Users can update categories
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'categories' AND policyname = 'Users can update categories') THEN
    CREATE POLICY "Users can update categories" ON categories
      FOR UPDATE USING (true);
  END IF;

  -- Users can delete categories
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'categories' AND policyname = 'Users can delete categories') THEN
