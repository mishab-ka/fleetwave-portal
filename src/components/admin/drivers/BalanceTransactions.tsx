  ...
  const fetchTransactions = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('driver_balance_transactions').select('*').eq('user_id', driverId).order('created_at', {
        ascending: false
      });
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <div className="text-sm text-muted-foreground">
            {currentBalance < 0 ? "Amount due from driver" : currentBalance > 0 ? "Amount available to driver" : "No pending balance"}
          </div>
        </CardHeader>
        <CardContent>
          ...
          {isAdding && <div className="mt-4 p-4 border rounded-md space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Button onClick={handleAddTransaction} disabled={isAdding}>
                  Add Transaction
                </Button>
              </div>
            </div>}
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <div className="flex justify-center py-8">
              ...
              return <div key={transaction.id} className="flex items-start p-3 border rounded-md">
                      <div className={`p-2 rounded-full mr-3 ${isPositiveTransaction ? 'bg-green-100' : 'bg-red-100'}`}>
                        {isPositiveTransaction ? <ArrowUp className="h-5 w-5 text-green-600" /> : <ArrowDown className="h-5 w-5 text-red-600" />}
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        {transaction.description && <p className="text-sm text-muted-foreground mt-1">
